...
class Router
{
     ...
     *
     * @param string $pattern  A pattern to match
     * @param array  $defaults The defaults for the :placeholders
     *
     * @return object \Sleepy\Route()
     */
    public static function mvc($pattern, $defaults = array())
    {
        self::route(
            $pattern,
            function ($route) use ($defaults) {
                // set default for defaults... (-_-)
                $defaults['controller'] = (array_key_exists('controller', $defaults))
                    ? $defaults['controller']
                    : 'home';
                $defaults['action'] = (array_key_exists('action', $defaults))
                    ? $defaults['action']
                    : 'index';
                $defaults['id'] = (array_key_exists('id', $defaults))
                    ? $defaults['id']
                    : '';
                ...
                $controller = (array_key_exists('controller', $route->params))
                    ? $route->params['controller']
                    : $defaults['controller'];
                $action = (array_key_exists('action', $route->params))
                    ? $route->params['action']
                    : $defaults['action'];
                $id = (array_key_exists('id', $route->params))
                    ? $route->params['id']
                    : $defaults['id'];
                    ...
                    if (class_exists($controller)) {
                        ...
                        if (method_exists($c, $action)) {
                            ...
                            throw new RouteNotFound(
                                "Router: Action ($action) does not exist in "
                                . "Controller ($controller)."
                            );
                        }
                    } else {
                        throw new RouteNotFound(
                            "Router: Controller ($controller) does not exist."
                        );
                    }
                } else {
                    throw new RouteNotFound(
                        "Router: Controller File ($controller_file) does not exist."
                    );
                }
            }
        );
        ...
        if (file_exists($controller_file)) {
            ...
            if (class_exists($controller)) {
                ...
                if (method_exists($c, $action)) {
                    ...
                    throw new \Exception(
                        "Router: Action ($action) does not exist."
                    );
                }
            } else {
                throw new \Exception(
    ...
    public static function start($currentPath='')
    {
        ...
        foreach (self::$_routes as $route) {
            $route->method = (isset($_SERVER['REQUEST_METHOD']))
...
class Route
{
    ...
    private function _isPlaceholder($string)
    {
        return substr($string, 0, 2)
            == '{{' && substr($string, strlen($string) - 2, 2) == '}}';
    }
    ...
    private function _runFilters($key, $string)
    {
        if (class_exists('Hook')) {
            ...
            $string = Hook::addFilter(
                'route_' . $this->name . '_parameters',
                $string
            );
            $string = Hook::addFilter(
                'route_' . $this->name . '_parameter_' . $key,
                $string
            );
        }
    ...
    public function execute()
    {
        ...
        if (Router::$routeFound) {
            ...
            if (count(Router::$parameters) == count($pattern)
                || $this->_hasWildcard($rawPattern)
            ) {
                // Check for matches, stop if we have a problem
                foreach ($pattern as $idx => $value) {
                    // Store the variable
                    if ($this->_isPlaceholder($value)) {
                        if (!$this->_storeVariable(
                            $value,
                            @Router::$parameters[$idx]
                        )
                        ) {
                            $noMatch = true;
                            break;
                    ...
                    if ($value == '*') {
                        $this->splat = implode(
                            Router::$delimiter,
                            array_slice(Router::$parameters, $idx)
                        );
                        break;
                    }

                    // If something doesn't match then stop the route.
                    if (!isset(Router::$parameters[$idx])
                        || $value != Router::$parameters[$idx]
                    ) {
                        $noMatch = true;
                        break;
                    }
